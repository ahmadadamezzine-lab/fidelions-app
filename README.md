...
16. **Conseiller IA**, rangé dans l'onglet **Statistiques** : un vrai fil de discussion, pas juste une analyse en un clic. Le commerçant enregistre son menu (texte, `.txt`, PDF, ou simple photo prise au téléphone), puis pose n'importe quelle question — "analyse mon menu et propose-moi des promos", "quelles idées pour le lundi midi ?", etc. — en joignant au besoin une photo/PDF à n'importe quel message. Une vraie IA (Google Gemini, gratuite, avec secours automatique Groq) répond en connaissant déjà le menu et l'offre du commerce. Juste en dessous, "Ton offre actuelle" est un texte 100% libre, et chaque réponse du conseiller peut y être ajoutée en un clic. Voir l'étape 3ter ci-dessous pour activer la vraie IA (gratuite, sans carte bancaire) et le paragraphe "Fiabilité du Conseiller IA" juste après : même sans aucune clé configurée, le conseiller répond toujours, avec au minimum quelques conseils génériques en tout dernier recours.
...
## Étape 3ter — Activer la vraie IA pour le Conseiller IA (gratuit, sans carte bancaire)

Sans cette étape, le Conseiller IA fonctionne quand même (voir "Fiabilité du Conseiller IA" ci-dessous — il répond toujours, même sans aucune clé), mais avec seulement des conseils génériques, moins fins qu'une vraie IA. Cette clé est gratuite chez Google, sans carte bancaire à saisir.
...
### Le Conseiller IA — un vrai fil de discussion, pas une analyse en un clic

Contrairement à l'ancienne carte "Analyser avec l'IA" (un seul aller-retour figé), le Conseiller IA (onglet Statistiques) est un vrai chat : le commerçant pose une question, l'IA répond en connaissant déjà le menu/l'offre/la récompense déjà enregistrés sur le compte, puis il peut enchaîner d'autres questions dans la même conversation. Une photo ou un PDF peut être joint à N'IMPORTE QUEL message, pas seulement le premier.

### Fiabilité du Conseiller IA — toujours une réponse, jamais un message d'erreur

1. **La vraie IA telle quelle** — Gemini, avec bascule automatique sur Groq (ci-dessus) si Gemini est à quota ou indisponible. Reçoit l'historique de la conversation + le contexte du commerce à chaque message.
2. **Pour une photo, si l'étape 1 échoue** : son texte est lu directement dans le navigateur du commerçant (reconnaissance de texte/OCR, via la bibliothèque open source Tesseract.js — aucun serveur, aucun quota pour cette étape), puis la vraie IA est retentée en mode texte, qui dispose d'un quota bien plus large qu'en mode photo.
3. **Filet de secours final** — géré côté serveur (`lib/advisor.js`) : si les deux IA sont indisponibles, une réponse de repli générique mais toujours utile est renvoyée, jamais un message d'erreur bloquant.

Rien à configurer pour ce comportement : il est actif d'office, avec ou sans `GEMINI_API_KEY`/`GROQ_API_KEY`. Conformément à la politique de confidentialité (`/confidentialite`), le texte/photo envoyé au conseiller peut être transmis sur Internet au service IA utilisé (Gemini ou Groq) — seule l'étape de lecture de photo (OCR) se fait localement, sans rien envoyer à un tiers.
...
- Le Conseiller IA ne renvoie jamais d'erreur bloquante côté commerçant (voir "Fiabilité du Conseiller IA" ci-dessus) — mais si une réponse est marquée "réponse de repli" au lieu d'une vraie réponse IA, c'est que ni Gemini ni Groq n'ont répondu (clé manquante, quota des deux atteint en même temps, ou service temporairement surchargé). Vérifie `GEMINI_API_KEY`/`GROQ_API_KEY` dans Vercel (étape 3ter), ou réessaie simplement quelques minutes plus tard.
