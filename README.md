...
9. Tu peux donner un accès limité à un employé (juste ajouter des tampons, sans les stats ni les campagnes) avec un 2e mot de passe séparé (`CASHIER_PASSWORD`) — ou, en mode "vraie équipe", un système complet dans l'onglet "Employés" de `/commercant` : un seul lien à envoyer (SMS/WhatsApp) à toute l'équipe, puis chaque employé s'identifie sur ce lien avec son propre code personnel à 4 chiffres. Pour chaque employé, tu choisis : les jours où il peut se connecter, une plage horaire optionnelle (ex. 9h-15h), et les rubriques auxquelles il a accès en plus du scan (liste des clients, statistiques, envoi de campagnes) — de quoi avoir plusieurs types d'employés, certains avec plus de responsabilités que d'autres. Un code hors de ses jours/horaires autorisés, ou désactivé, est refusé comme s'il n'existait pas ; deux employés ne peuvent pas avoir le même code (le site refuse et te le signale). Tu peux régénérer le lien commun à tout moment (coupe l'accès à toute l'équipe d'un coup), ou désactiver/modifier/supprimer un employé individuellement sans toucher aux autres.
...
15. Carte "Analyse du menu & suggestions (IA)", rangée dans l'onglet **Statistiques** de `/commercant` (avec les graphes, c'est le même genre d'analyse) : importe ton menu tel quel — texte collé/écrit, ou glisse-dépose un fichier `.txt`, PDF, ou simple photo prise au téléphone directement dans la zone prévue (ou clique dessus pour choisir un fichier) — et une vraie IA (Google Gemini, gratuite) le lit et le comprend toute seule (pas de règles écrites à la main), puis propose des idées de promotions concrètes qui citent tes propres plats et prix. Juste en dessous, "Ton offre actuelle" est un texte 100% libre : tu peux reprendre les suggestions de l'IA en un clic ou tout écrire/modifier toi-même, directement depuis le site — c'est ce texte que tu gardes et actualises comme ton offre du moment. Voir l'étape 3ter ci-dessous pour activer la clé IA (gratuite, sans carte bancaire) ; tant qu'elle n'est pas configurée, une analyse basique (règles simples) prend le relais automatiquement pour le texte collé/écrit — mais PDF et photo demandent la vraie clé IA. En cas de pic de charge chez Google (erreur 503), le site réessaie automatiquement une fois avant d'abandonner.
16. `/commercant` est organisé en rubriques dans un vrai menu latéral fixé tout à gauche de l'écran (comme Sydely) sur ordinateur/tablette — le contenu utilise alors toute la largeur de l'écran, plus de cadre centré avec du vide sur les côtés — et en rangée d'onglets défilante sur mobile, où la place manque pour une barre latérale. Rubriques : **Aperçu**, **Partager**, **Clients**, **Notifications**, **Ma carte**, **Récompenses**, **Employés**, **Géolocalisation**, **Statistiques**, **API & développeurs**, puis un groupe **Compte** séparé : **Établissement**, **Abonnement**, **Support**, **Paramètres**.
17. Onglet **Partager** : le QR code d'inscription et le lien public du site, affichés directement dans le tableau de bord (avec un bouton pour télécharger l'image à imprimer et un bouton pour copier le lien) — plus besoin d'ouvrir une page à part.
18. Onglet **Récompenses** (anciennement "Fidélité") : choisis juste le mot affiché ("tampons" ou "points"), puis écris librement autant de récompenses que tu veux, chacune avec son propre seuil (ex : 20 tampons → pizza offerte, 30 → pizza + boisson offertes) — comme chez Sydely. Il y a toujours au moins une récompense modifiable (impossible de tout supprimer), et rien n'empêche d'en avoir plusieurs pour donner plusieurs objectifs à tes clients plutôt qu'une seule récompense.
19. Onglet **Statistiques** : en plus de l'analyse IA du menu, 3 chiffres clés (points/tampons de la semaine, nouveaux clients, récompenses du mois, avec leur évolution en %) et 4 graphes (distribution par jour sur 14 jours, heures de pointe, jours de la semaine les plus actifs, nouveaux clients par semaine).
20. Onglet **Ma carte** : change la couleur, le logo et la bannière affichés sur la carte Google Wallet de tes clients directement depuis le site (stocké sur Vercel Blob, gratuit — voir étape 3quater) — les cartes déjà distribuées se mettent à jour toutes seules, sans rien demander aux clients.
21. Onglet **Géolocalisation** (anciennement "Proximité") : tape l'adresse de ton restaurant — un menu déroulant propose des adresses françaises réelles au fil de la frappe (autocomplétion officielle, gratuite), donc pas besoin de connaître l'orthographe exacte ni le code postal — et Google Wallet avertit tout seul (vraie notification native sur le téléphone) les clients équipés qui passent à proximité — aucune app ni réglage supplémentaire à installer, ni chez toi ni chez le client. Tu peux aussi écrire un message personnalisé (ex. "On a hâte de vous voir ! 👋") qui reste affiché en permanence sur la carte de tes clients dans l'app Wallet : c'est honnête de le préciser, Google ne permet pas (à ce jour, via son API publique) de personnaliser le texte de sa notification-popup native de proximité elle-même — ton message est donc affiché en continu sur la carte plutôt que seulement au moment du passage à proximité.
22. Onglet **Support** (anciennement "Aide") : les réponses aux blocages les plus fréquents (notifications, caméra, email, IA…), consultables à toute heure — pas de faux chat en ligne qui resterait sans réponse.
23. Rubriques **API & développeurs**, **Établissement**, **Abonnement** et **Paramètres** : présentes dans le menu pour matcher exactement la structure demandée, mais annoncées honnêtement "bientôt disponible" — il n'y a pas encore de vraie fonctionnalité derrière (pas d'API publique, pas de facturation, pas de gestion multi-établissement). Rien ne prétend fonctionner alors que ça ne fonctionne pas.
...
Sans cette étape, tout le reste fonctionne normalement — c'est juste l'onglet "Ma carte" (logo/bannière) qui affichera une erreur si tu essaies d'envoyer une image. La couleur, elle, fonctionne sans rien à activer.
...
- Le lien employé (onglet "Équipe") ne fonctionne plus → il a probablement été régénéré depuis (volontairement, ou par erreur) : renvoie le nouveau lien affiché dans cet onglet à ton employé.
- Un employé se fait refuser son code ("Code incorrect, ou accès en dehors de tes horaires autorisés") alors qu'il est sûr de son code → vérifie dans l'onglet "Équipe" que son compte est bien "Actif", et que le jour/l'heure actuels sont bien dans les jours/la plage horaire que tu lui as autorisés — c'est voulu : hors de son créneau, l'accès se coupe vraiment, pas juste visuellement.
- En créant/modifiant un employé, le site refuse le code avec "Ce code est déjà utilisé par ..." → deux employés ne peuvent pas avoir le même code à 4 chiffres (sinon impossible de savoir lequel des deux vient de se connecter) ; choisis-en un autre pour ce nouvel employé.
- La couleur/le logo/la bannière ne se voient pas tout de suite sur une carte déjà installée → laisse quelques minutes à Google pour propager le changement à toutes les cartes déjà distribuées ; un nouveau client qui s'inscrit après voit la mise à jour immédiatement.
