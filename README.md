...
9. Tu peux donner un accès limité à un employé (juste ajouter des tampons, sans les stats ni les campagnes) avec un 2e mot de passe séparé (`CASHIER_PASSWORD`) — ou, en mode "vraie équipe", un système complet dans l'onglet "Équipe" de `/commercant` : un seul lien à envoyer (SMS/WhatsApp) à toute l'équipe, puis chaque employé s'identifie sur ce lien avec son propre code personnel à 4 chiffres. Pour chaque employé, tu choisis : les jours où il peut se connecter, une plage horaire optionnelle (ex. 9h-15h), et les rubriques auxquelles il a accès en plus du scan (liste des clients, statistiques, envoi de campagnes) — de quoi avoir plusieurs types d'employés, certains avec plus de responsabilités que d'autres. Un code hors de ses jours/horaires autorisés, ou désactivé, est refusé comme s'il n'existait pas. Tu peux régénérer le lien commun à tout moment (coupe l'accès à toute l'équipe d'un coup), ou désactiver/modifier/supprimer un employé individuellement sans toucher aux autres.
