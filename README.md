...
9. Tu peux donner un accès limité à un employé (juste ajouter des tampons, sans les stats ni les campagnes) avec un 2e mot de passe séparé (`CASHIER_PASSWORD`) — ou, encore plus restreint, un simple lien à envoyer (SMS/WhatsApp) depuis l'onglet "Équipe" de `/commercant` : celui qui l'ouvre peut seulement scanner une carte pour ajouter un tampon/point, sans jamais voir la liste de tes clients. Tu peux régénérer ce lien à tout moment (l'ancien arrête aussitôt de fonctionner).
...
16. `/commercant` est organisé en onglets : Aperçu, Fidélité, Statistiques, Ma carte, Proximité, Équipe, Campagnes, Clients, Aide.
17. Onglet **Fidélité** : choisis entre une carte à tampons classique (un seul seuil) ou des **points cumulés à plusieurs paliers** (ex : 50 points → café offert, 150 points → repas offert) — comme chez Sydely, avec autant de paliers que tu veux.
...
20. Onglet **Proximité** : tape l'adresse de ton restaurant, et Google Wallet avertit tout seul (vraie notification native sur le téléphone) les clients équipés qui passent à proximité — aucune app ni réglage supplémentaire à installer, ni chez toi ni chez le client.
...
1. Va sur `vercel.com`, ouvre ton projet `fidelions-app`.
2. Onglet `Storage` (en haut) → `Create Database`.
3. Choisis `Blob` (offre gratuite).
4. Donne-lui un nom (ex. `fidelions-images`) → `Create`.
5. Sur l'écran suivant, coche ton projet `fidelions-app` puis `Connect` — Vercel ajoute automatiquement la variable `BLOB_READ_WRITE_TOKEN` à ton projet, exactement comme pour la base de données à l'étape 3 : tu n'as rien à copier toi-même.
