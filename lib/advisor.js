...
// Ces identifiants de modèle doivent être vérifiés de temps en temps : Google
// et Groq retirent régulièrement d'anciens modèles/alias, qui se mettent
// alors à échouer en boucle (404) — ce qui, vu du commerçant, ressemble à
// tort à un problème de quota puisque le message affiché reste générique.
// Vérifiés le 22/09/2026 (voir historique) : "gemini-pro-latest" et
// "gemini-flash-lite-latest" n'existaient plus (100% d'échec sur Gemini) ;
// côté Groq, les deux modèles vision étaient tous les deux retirés (100%
// d'échec dès qu'une photo était jointe) et "llama-3.1-8b-instant" était en
// cours de retrait. Remplacés par les identifiants stables en vigueur à date.
const GEMINI_MODEL_CASCADE = ["gemini-3.5-flash", "gemini-3.5-flash-lite", "gemini-3.1-flash-lite"];
const GROQ_TEXT_MODEL_CASCADE = ["llama-3.3-70b-versatile", "openai/gpt-oss-20b", "openai/gpt-oss-120b"];
const GROQ_VISION_MODEL_CASCADE = ["qwen/qwen3.8-27b"];
...
export async function askAdvisor({ messages, file, context }) {
  ...
  // Raisons réelles des deux échecs, gardées uniquement pour le cas où l'on
  // tombe sur le filet de secours final — remontées jusqu'au commerçant
  // (voir pages/commercant.js, ligne "détails") pour pouvoir diagnostiquer
  // sans avoir besoin de regarder les logs Vercel à chaque fois.
  const reasons = [];
  ...
  if (geminiConfigured) {
    try {
      ...
      reasons.push(`Gemini : ${err.message}`);
    }
  } else {
    reasons.push("Gemini : GEMINI_API_KEY non configurée.");
  }
  ...
  if (groqConfigured) {
    try {
      ...
      reasons.push(`Groq : ${err.message}`);
    }
  } else {
    reasons.push("Groq : GROQ_API_KEY non configurée.");
  }

  return {
    reply: localAdvisorFallback(context || {}),
    provider: "local",
    fallback: true,
