 ...
 */
async function patchLoyaltyClass(data) {
  if (!CLASS_ID) {
...
export async function patchLoyaltyClassPointsLabel(label) {
  await patchLoyaltyClass({ loyaltyPoints: { label } });
}
...
export async function patchLoyaltyClassLocations(locations) {
  ...
  await patchLoyaltyClass({ locations: cleanLocations });
}
