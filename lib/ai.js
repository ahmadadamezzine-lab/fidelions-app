      ...
      res = await fetch(geminiUrlFor(model), {
        ...
        body: JSON.stringify({
          ...
          generationConfig: {
            ...
            // exprès ci-dessous, pour de vraies stratégies réfléchies et
            // pas des idées génériques) ET la réponse finale, plus longue
            // qu'avant (6-8 idées/tips de plusieurs phrases). D'où un
            // budget large — c'est aussi ce qui avait causé le bug "JSON
            // coupé en plein milieu" remonté par Adam quand ce budget
            // était trop juste (1024) : mieux vaut prévoir large que
            // retomber sur l'analyse basique.
            maxOutputTokens: 16384,
            // Réflexion volontairement ACTIVÉE ici (contrairement à un
            // premier réglage plus conservateur) : Adam veut que l'IA
            // "réfléchisse" et élabore de vraies stratégies au lieu de
            // sortir des idées interchangeables — thinkingBudget: -1 (les
            // modèles Gemini 2.5) laisse le modèle décider lui-même
            // combien réfléchir selon la complexité du menu, et
            // thinkingLevel: "high" fait la même chose sur les modèles
            // Gemini 3.x. Les deux champs coexistent pour couvrir toute
            // la cascade ci-dessus quel que soit le modèle réel derrière
            // chaque alias.
            thinkingConfig: { thinkingBudget: -1, thinkingLevel: "high" },
