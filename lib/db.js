 ...
 * voir getSubscriptionAccess plus bas. Un compte SANS AUCUN enregistrement
 * ici (`stored` === null) est un compte créé avant l'existence de ce
 * champ : il est traité comme "actif" par défaut plutôt que bloqué au
 * lancement de cette fonctionnalité (pas question de couper l'accès à des
 * commerçants déjà en place du jour au lendemain).
 */
...
export async function saveSubscriptionChoice(merchantId, { posCount, billingCycle, status, trialEndsAt }) {
  const redis = getRedis();
...
/**
 * Changement manuel du statut d'abonnement — utilisé uniquement par Adam,
 * via /api/admin-set-subscription (protégé par SESSION_SECRET, même
 * principe que /api/admin-reset-password), une fois le paiement reçu (→
 * "actif") ou pour couper un compte (→ "suspendu"/"essai").
 */
 ...
 * dont la date limite est dépassée, ou un compte "suspendu", n'est plus
 * `allowed`, SANS qu'Adam ait quoi que ce soit à faire — le blocage est
 * automatique. Utilisé (1) à chaque connexion via /api/clients pour
 * afficher l'écran de paiement côté commerçant, et (2) juste avant les
...
export async function getSubscriptionAccess(merchantId) {
  ...
  const daysLeft =
    sub.status === "essai" && sub.trialEndsAt
      ...
      : null;
  return { allowed, status: sub.status, trialEndsAt: sub.trialEndsAt, daysLeft };
}
