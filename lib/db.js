 ...
 * point (ensemble = un même client ne compte qu'une fois, même scanné
 * plusieurs fois par le même employé).
 */
export async function recordEmployeeStamp(merchantId, employeeId, objectId) {
  if (!employeeId) return;
  const redis = getRedis();
  await redis.sadd(`employee-clients:${merchantId}:${employeeId}`, objectId);
}

/** Chaque avis Google obtenu (bonus, voir markClientReview) incrémente ce compteur. */
export async function recordEmployeeReview(merchantId, employeeId) {
  if (!employeeId) return;
  const redis = getRedis();
  await redis.incr(`employee-reviews:${merchantId}:${employeeId}`);
}

/**
 * Classement complet de l'équipe, trié par nombre de clients fidélisés
 * (puis par avis en cas d'égalité) — affiché dans l'onglet Équipe.
 */
  ...
  const rows = await Promise.all(
    employees.map(async (e) => {
      const [clientsCount, reviewsCount] = await Promise.all([
        redis.scard(`employee-clients:${merchantId}:${e.id}`),
        redis.get(`employee-reviews:${merchantId}:${e.id}`),
      ]);
      return {
        id: e.id,
        name: e.name,
