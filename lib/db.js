 ...
 * lien lui-même (employee-token ci-dessus) reste commun à toute l'équipe ;
 * c'est le code à 4 chiffres qui identifie QUI l'utilise et ce qu'il peut
 * faire, exactement comme sur les captures que tu as envoyées.
 */
const EMPLOYEES_KEY = `employees:${RESTAURANT_ID}`;
const ALL_DAYS = ["lun", "mar", "mer", "jeu", "ven", "sam", "dim"];

export async function getEmployees() {
  const redis = getRedis();
  const list = await redis.get(EMPLOYEES_KEY);
...
export async function upsertEmployee(input) {
  ...
  const redis = getRedis();
  await redis.set(EMPLOYEES_KEY, list);
  return clean;
}
...
export async function deleteEmployee(id) {
  ...
  const redis = getRedis();
  await redis.set(EMPLOYEES_KEY, next);
  return next;
}
 ...
 */
export async function findEmployeeByPin(pin) {
