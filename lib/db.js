...
export const DEFAULT_REVIEW_BONUS_POINTS = 3;
...
export async function createMerchant({
  ...
  // Index global de tous les commerces — sert uniquement à la relance
  // automatique "client inactif" (voir getNotificationSettings/cron
  // winback-notifications.js plus bas), qui doit pouvoir parcourir tous
  // les comptes chaque jour sans connaître leurs identifiants à l'avance.
  await redis.sadd("merchants:all", id);

  return record;
}

/**
 * Tous les identifiants de commerce existants — utilisé uniquement par la
 * relance automatique "client inactif" (voir plus bas), qui doit scanner
 * chaque compte une fois par jour. Les comptes créés avant l'ajout de
 * l'index ci-dessus (`merchants:all`) ne seraient pas couverts ; peu grave
 * en pratique (winback est un bonus, pas une fonctionnalité critique), et
 * évite une migration lourde pour un cas déjà rare.
 */
export async function listAllMerchantIds() {
  const redis = getRedis();
  return (await redis.smembers("merchants:all")) || [];
}
...
export async function createClient({ merchantId, objectId, prenom, email, telephone, referredByCode }) {
  ...
  const record = {
    ...
    createdAt: Date.now(),
    // Compte les passages en caisse pour les automatisations (voir
    // maybeScheduleReviewRequest dans lib/notifications.js) : la création
    // de la carte compte comme le 1er passage ("le scan qui crée la
    // carte"), chaque scan suivant (voir addPoints ci-dessous, appelé
    // depuis /api/add-stamp avec countVisit=true) l'incrémente.
    visitCount: 1,
    reviewRequestCount: 0,
    reviewRequestSentAt: null,
    reviewRequestPendingUntil: null,
...
/**
 * Retrouve un client par son seul objectId, SANS vérifier le restaurant
 * propriétaire (contrairement à getClient ci-dessus). Réservé à deux cas où
 * on ne connaît que l'objectId au moment de l'appel : le lien de suivi de
 * clic d'un email de demande d'avis (pages/api/review-redirect.js) et le
 * job programmé qui envoie cette demande (voir lib/notifications.js).
 * L'objectId lui-même joue déjà le rôle de jeton (c'est la valeur encodée
 * dans le QR code de la carte, voir lib/wallet.js) : personne d'autre que
 * le client et Fidélions ne le connaît.
 */
export async function getClientByObjectId(objectId) {
  const redis = getRedis();
 ...
 *
 * `countVisit` : true uniquement quand l'appel vient d'un vrai scan en
 * caisse (/api/add-stamp) — incrémente visitCount, utilisé par les
 * automatisations de demande d'avis (voir lib/notifications.js). Laissé à
 * false pour le bonus de parrainage (/api/create-pass.js) : le parrain
 * gagne un point sans être physiquement passé, ça ne doit pas compter
 * comme une visite.
 */
export async function addPoints(merchantId, objectId, delta = 1, { countVisit = false } = {}) {
  const redis = getRedis();
...
/**
 * Enregistre (ou remplace) l'abonnement de notifications push web d'un
 * client — un seul par carte, la page carte (/r/[slug]) resouscrit à
 * chaque clic sur "Activer les notifications", ce qui écrase l'ancien.
 */
export async function saveClientPushSubscription(merchantId, objectId, subscription) {
  const redis = getRedis();
  const record = await getOwnedClientRecord(redis, merchantId, objectId);
  if (!record) return null;
...
/**
 * Oublie l'abonnement push d'un client — appelé quand un envoi échoue avec
 * un abonnement expiré/révoqué (voir lib/webpush.js), pour ne pas
 * réessayer indéfiniment un appareil qui ne recevra plus jamais rien.
 */
export async function removeClientPushSubscription(merchantId, objectId) {
  const redis = getRedis();
  const record = await getOwnedClientRecord(redis, merchantId, objectId);
  if (!record) return null;
  record.pushSubscription = null;
  await redis.set(`client:${objectId}`, record);
  return record;
}
...
export async function getLoyaltySettings(merchantId) {
  ...
  // Bonus (en points) accordé une seule fois par client quand un avis
  // Google est déclaré en caisse (voir markClientReview / add-stamp.js) —
  // réglable par le commerçant depuis l'onglet Fidélité, 3 points par
  // défaut.
  const reviewBonusPoints =
    settings && Number(settings.reviewBonusPoints) >= 0
      ? Math.round(Number(settings.reviewBonusPoints))
      : DEFAULT_REVIEW_BONUS_POINTS;
  return { tiers, mode, pointsConfig, reviewBonusPoints };
}

export async function updateLoyaltySettings(merchantId, { tiers, mode, pointsConfig, reviewBonusPoints }) {
  const redis = getRedis();
  ...
  // Borné à 50 : un bonus plus élevé n'a pas vraiment de sens pour un
  // avis (et évite une saisie fantaisiste type "9999").
  const cleanReviewBonusPoints =
    reviewBonusPoints !== undefined && Number(reviewBonusPoints) >= 0
      ? Math.min(50, Math.round(Number(reviewBonusPoints)))
      : current.reviewBonusPoints !== undefined
      ? current.reviewBonusPoints
      : DEFAULT_REVIEW_BONUS_POINTS;
  ...
  const next = {
    ...
    reviewBonusPoints: cleanReviewBonusPoints,
    rewardThreshold: cleanTiers[0].threshold,
    rewardLabel: cleanTiers[0].label,
  };
  await redis.set(`settings:${merchantId}`, next);
  return { tiers: next.tiers, mode: next.mode, pointsConfig: next.pointsConfig, reviewBonusPoints: next.reviewBonusPoints };
}
 ...
 * lib/pricing.js) — affichée dans l'onglet Abonnement.
 *
 * Porte aussi le statut d'abonnement réel (`status` / `trialEndsAt` /
 * `activeUntil`) et les identifiants Stripe du commerce
 * (`stripeCustomerId` / `stripeSubscriptionId`) — voir getSubscriptionAccess
 * plus bas et lib/stripe.js pour comment ils sont alimentés tout seuls par
 * les paiements Stripe. Un compte SANS AUCUN enregistrement ici (`stored`
 * === null) est un compte créé avant l'existence de ce champ : il est
 * traité comme "actif" par défaut plutôt que bloqué au lancement de cette
 * fonctionnalité (pas question de couper l'accès à des commerçants déjà en
 * place du jour au lendemain).
 */
export async function getSubscriptionChoice(merchantId) {
  ...
  return {
    ...
    status: (stored && stored.status) || (stored ? "essai" : "actif"),
    trialEndsAt: (stored && stored.trialEndsAt) || null,
    stripeCustomerId: (stored && stored.stripeCustomerId) || null,
    stripeSubscriptionId: (stored && stored.stripeSubscriptionId) || null,
    activeUntil: (stored && stored.activeUntil) || null,
  };
}

export async function saveSubscriptionChoice(
  merchantId,
  { posCount, billingCycle, status, trialEndsAt, stripeCustomerId, stripeSubscriptionId, activeUntil } = {}
) {
  const redis = getRedis();
  ...
  const next = {
    ...
    stripeCustomerId: stripeCustomerId !== undefined ? stripeCustomerId : current.stripeCustomerId,
    stripeSubscriptionId: stripeSubscriptionId !== undefined ? stripeSubscriptionId : current.stripeSubscriptionId,
    activeUntil: activeUntil !== undefined ? activeUntil : current.activeUntil,
  };
...
/**
 * Relie un commerce à son client Stripe (créé/retrouvé au premier
 * paiement) — index inverse nécessaire pour retrouver le commerce à partir
 * d'un événement webhook `invoice.paid`, qui ne porte que l'identifiant du
 * client Stripe, jamais le merchantId Fidélions (voir
 * pages/api/stripe-webhook.js).
 */
export async function linkMerchantToStripeCustomer(merchantId, stripeCustomerId) {
  const redis = getRedis();
  if (stripeCustomerId) {
    await redis.set(`stripe-customer:${stripeCustomerId}`, merchantId);
  }
  return saveSubscriptionChoice(merchantId, { stripeCustomerId });
}

export async function getMerchantIdByStripeCustomer(stripeCustomerId) {
  if (!stripeCustomerId) return null;
  const redis = getRedis();
  return (await redis.get(`stripe-customer:${stripeCustomerId}`)) || null;
}

const SUBSCRIPTION_STATUSES = ["essai", "actif", "suspendu"];

/**
 * Changement MANUEL du statut d'abonnement — dépannage exclusif d'Adam via
 * /api/admin-set-subscription (protégé par SESSION_SECRET, même principe
 * que /api/admin-reset-password). Le chemin normal, entièrement
 * automatique, passe par Stripe (voir saveSubscriptionChoice appelé depuis
 * pages/api/stripe-webhook.js à chaque paiement réussi).
 */
export async function setSubscriptionStatus(merchantId, status) {
  if (!SUBSCRIPTION_STATUSES.includes(status)) {
    throw new Error(`Statut d'abonnement invalide : ${status} (attendu : ${SUBSCRIPTION_STATUSES.join(", ")}).`);
  }
  return saveSubscriptionChoice(merchantId, { status });
}

/**
 * Calcule si le commerce a le droit d'utiliser le Service maintenant —
 * c'est ce qui matérialise le "verrou d'abonnement" : un compte "essai"
 * dont la date limite est dépassée, un compte "suspendu", ou un compte
 * "actif" dont la période payée sur Stripe (`activeUntil`) est dépassée,
 * n'est plus `allowed`, SANS qu'Adam ait quoi que ce soit à faire — le
 * blocage est automatique. Utilisé (1) à chaque connexion via /api/clients
 * pour afficher l'écran de paiement côté commerçant, et (2) juste avant les
 * deux actions qui font vivre le programme de fidélité (ajout de point,
 * création de carte) pour empêcher un compte non payant de continuer à
 * en tirer de la valeur même sans se reconnecter au tableau de bord.
 *
 * `activeUntil` est prolongé tout seul à chaque prélèvement Stripe réussi
 * (voir pages/api/stripe-webhook.js, événement `invoice.paid`) : tant que
 * les paiements mensuels continuent d'arriver, l'accès ne s'arrête jamais.
 * Dès que l'engagement se termine (Stripe arrête de lui-même de prélever,
 * voir lib/stripe.js) ou qu'un paiement échoue, `activeUntil` n'avance
 * plus, et l'accès se reverrouille automatiquement une fois la date
 * dépassée — exactement le même mécanisme que la fin d'essai. Un compte
 * "actif" sans `activeUntil` (activé à la main avant l'intégration Stripe,
 * ou par dépannage via /api/admin-set-subscription) reste illimité, comme
 * avant.
 */
export async function getSubscriptionAccess(merchantId) {
  const sub = await getSubscriptionChoice(merchantId);
  let allowed = true;
  if (sub.status === "suspendu") {
    allowed = false;
  } else if (sub.status === "essai") {
    allowed = !sub.trialEndsAt || Date.now() < sub.trialEndsAt;
  } else if (sub.status === "actif") {
    allowed = !sub.activeUntil || Date.now() < sub.activeUntil;
  }
  const daysLeft =
    sub.status === "essai" && sub.trialEndsAt
      ? Math.max(0, Math.ceil((sub.trialEndsAt - Date.now()) / (24 * 3600 * 1000)))
      : null;
  return { allowed, status: sub.status, trialEndsAt: sub.trialEndsAt, activeUntil: sub.activeUntil, daysLeft };
}

// ---------------------------------------------------------------------
// Limitation de fréquence (rate limiting) sur les endpoints publics
// ---------------------------------------------------------------------

/**
 * Limiteur simple à fenêtre fixe, basé sur Redis (déjà en place, pas de
 * nouvelle dépendance) : incrémente un compteur par clé (ex. IP + route)
 * et le fait expirer après `windowSeconds`. Renvoie `true` si la requête
 * est autorisée, `false` si la limite est dépassée pour cette fenêtre.
 * Volontairement approximatif (fenêtre fixe, pas glissante) : suffisant
 * pour dissuader un script de créer des centaines de fausses cartes ou de
 * tenter des connexions en boucle, pas conçu pour bloquer un attaquant
 * déterminé et distribué.
 */
...
/**
 * Nouvelle carte créée EN CAISSE par un employé (voir "Nouveau client" dans
 * l'écran de scan, pages/scan/[token].js -> /api/create-pass) — un client
 * qui s'inscrit lui-même via le lien public /r/[slug] n'est attribué à
 * aucun employé (pas d'authentification à ce stade-là).
 */
export async function recordEmployeeCardCreated(merchantId, employeeId) {
  if (!employeeId) return;
  const redis = getRedis();
  await redis.incr(`employee-cards:${merchantId}:${employeeId}:${currentMonthKey()}`);
}

/**
 * Montant (en €) saisi par l'employé en mode "points" (voir
 * /api/add-stamp) — cumulé pour donner un classement "CA généré", en plus
 * du nombre de clients fidélisés. N'a de sens qu'en mode "points" (en mode
 * "tampons" aucun montant n'est jamais saisi, ce compteur reste à 0).
 */
export async function recordEmployeeRevenue(merchantId, employeeId, amount) {
  if (!employeeId || !(Number(amount) > 0)) return;
  const redis = getRedis();
  await redis.incrbyfloat(`employee-revenue:${merchantId}:${employeeId}:${currentMonthKey()}`, Number(amount));
}

/**
 * Classement de l'équipe pour le mois en cours — clients fidélisés, avis
 * obtenus, cartes créées et CA généré, un objet par employé. Trié par
 * clients fidélisés par défaut (puis avis en cas d'égalité) ; le
 * commerçant peut re-trier côté écran (onglet Équipe) selon le critère de
 * son choix sans nouvel appel serveur, chaque ligne portant déjà les 4
 * compteurs. `monthKey` (ex: "2026-09") est renvoyé pour l'affichage.
 */
export async function getEmployeeLeaderboard(merchantId) {
  ...
  const rows = await Promise.all(
    employees.map(async (e) => {
      const [clientsCount, reviewsCount, cardsCount, revenueTotal] = await Promise.all([
        redis.scard(`employee-clients:${merchantId}:${e.id}:${monthKey}`),
        redis.get(`employee-reviews:${merchantId}:${e.id}:${monthKey}`),
        redis.get(`employee-cards:${merchantId}:${e.id}:${monthKey}`),
        redis.get(`employee-revenue:${merchantId}:${e.id}:${monthKey}`),
      ]);
      return {
        id: e.id,
        name: e.name,
...
export async function getEstablishmentInfo(merchantId) {
  ...
  return {
    ...
    // Lien direct vers la fiche Google du commerce (ex :
    // https://g.page/r/XXXX/review), utilisé pour le bouton "Laisser un
    // avis Google" affiché sur la carte Wallet et sur la page d'inscription
    // publique (pages/r/[slug].js) — voir lib/wallet.js.
    googleReviewUrl: (stored && stored.googleReviewUrl) || "",
...
export async function saveEstablishmentInfo(merchantId, data) {
  ...
  const next = {
    ...
    googleReviewUrl:
      data.googleReviewUrl !== undefined ? (data.googleReviewUrl || "").trim().slice(0, 300) : current.googleReviewUrl,
    description:
...
// ---------------------------------------------------------------------
// Cartes NFC/QR physiques (onglet "Partager" de la page d'accueil)
//
// Adam commande un lot de cartes en avance chez l'imprimeur, chacune
// gravée (puce NFC + QR) avec une URL DU TYPE fidelions-app.vercel.app/c/
// <code> — un code généré ici, pas un vrai lien de commerçant. Comme ça,
// le lot entier est identique pour l'imprimeur (numérotation simple,
// aucune donnée variable à lui fournir), et c'est seulement au moment où
// une carte est VENDUE qu'on la relie à un commerçant (voir assignCard) :
// pages/c/[code].js consulte cette table et redirige vers le bon
// /r/<slug> à ce moment-là. Une carte jamais vendue reste "libre" et peut
// être réassignée à un autre commerçant si besoin (erreur, carte perdue
// puis retrouvée, etc.) sans jamais toucher à l'impression.
// ---------------------------------------------------------------------

/**
 * Génère `quantity` codes de carte uniques (même alphabet que shortCode,
 * sans caractères ambigus — pensé pour être gravé/imprimé puis, au pire,
 * retapé à la main). Renvoie la liste des codes ET l'URL complète à
 * donner à l'imprimeur, prêtes à copier dans son formulaire de commande.
 */
export async function createCardBatch(quantity, { baseUrl } = {}) {
  const redis = getRedis();
  const n = Math.max(1, Math.min(500, Number(quantity) || 0));
  const clean = (baseUrl || "https://fidelions-app.vercel.app").replace(/\/+$/, "");
  const codes = [];
  for (let i = 0; i < n; i++) {
    let code;
    do {
      code = shortCode(6);
    } while (await redis.get(`card:${code}`));
    const record = {
      code,
      merchantSlug: null,
      merchantId: null,
    ...
    await redis.set(`card:${code}`, record);
    await redis.sadd("cards:all", code);
    codes.push(code);
  }
  return codes.map((code) => ({ code, url: `${clean}/c/${code}` }));
}
...
export async function getCard(code) {
  ...
  const redis = getRedis();
  return (await redis.get(`card:${clean}`)) || null;
}

/** Liste toutes les cartes déjà générées (assignées ou non), les plus récentes en premier. */
export async function listCards() {
  const redis = getRedis();
  const codes = await redis.smembers("cards:all");
  if (!codes.length) return [];
  const records = await Promise.all(codes.map((code) => redis.get(`card:${code}`)));
  return records.filter(Boolean).sort((a, b) => b.createdAt - a.createdAt);
}

/**
 * Relie une carte déjà imprimée à un commerçant (au moment de la vente).
 * Le commerçant est identifié par son slug — celui déjà visible dans son
 * propre lien /r/<slug>, facile à récupérer sans lui rien demander de
 * plus. Écrase une éventuelle affectation précédente (utile si une carte
 * est réattribuée après une erreur).
 */
export async function assignCard(code, merchantSlug) {
  const clean = (code || "").trim().toUpperCase();
  const card = await getCard(clean);
  if (!card) {
    throw new Error("Carte introuvable : vérifie le code.");
  }
  const merchant = await getMerchantBySlug(merchantSlug);
  if (!merchant) {
    throw new Error("Aucun commerçant avec ce lien/slug.");
  }
  const redis = getRedis();
  const next = {
    ...card,
  ...
  await redis.set(`card:${clean}`, next);
  return next;
}

/** Libère une carte (erreur d'attribution, carte perdue puis retrouvée...). */
export async function unassignCard(code) {
  const clean = (code || "").trim().toUpperCase();
  const card = await getCard(clean);
  if (!card) {
    throw new Error("Carte introuvable : vérifie le code.");
  ...
  const redis = getRedis();
  const next = { ...card, merchantSlug: null, merchantId: null, restaurantName: null, assignedAt: null };
  await redis.set(`card:${clean}`, next);
  return next;
}

// ---------------------------------------------------------------------
// Leads d'inscription abandonnée — relance automatique par email, façon
// "Vous vous êtes inscrit(e)... il ne vous reste qu'une étape" (l'email
// Fidelix montré par Adam). Le formulaire d'inscription de /commercant ne
// demande l'email qu'à la toute dernière étape (6/6) : c'est donc LE
// moment où on a enfin de quoi recontacter quelqu'un qui a rempli tout le
// reste (nom du commerce, mécanique de fidélité, formule) mais n'a pas
// cliqué sur "Valider" — voir saveSignupLead, appelée dès que ce champ
// email est renseigné (pages/commercant.js).
//
// Un SET Redis à part (`signup-leads:pending`) évite un KEYS/SCAN complet
// pour retrouver les leads à relancer : seuls les emails encore "en
// attente" y restent, et markSignupLeadCompleted/markSignupLeadReminderSent
// les en retirent dès qu'il n'y a plus besoin d'y repenser.
// ---------------------------------------------------------------------

const PENDING_LEADS_SET = "signup-leads:pending";

/**
 * Enregistre/actualise un lead en cours d'inscription. Appelée à chaque
 * fois que le champ email de l'étape 6 change — idempotent (une nouvelle
 * frappe met juste à jour restaurantName/posCount/billingCycle si le
 * commerçant les a changés entre-temps), et sans effet si l'inscription a
 * déjà été complétée entre-temps (on ne fait pas réapparaître un lead
 * "terminé" dans la liste d'attente).
 */
export async function saveSignupLead({ email, restaurantName, posCount, billingCycle }) {
  const cleanEmail = normalizeEmail(email);
  if (!cleanEmail || !cleanEmail.includes("@")) return null;

  const redis = getRedis();
  const key = `signup-lead:${cleanEmail}`;
  ...
  const next = {
    email: cleanEmail,
    restaurantName: (restaurantName || existing?.restaurantName || "").trim().slice(0, 60),
    posCount: posCount || existing?.posCount || null,
  ...
  // Pas la peine de re-rajouter au SET si un rappel est déjà parti — la
  // relance est volontairement à usage unique (voir le cron), pas un rappel
  // en boucle tant que le commerçant ne finalise pas.
  if (!next.reminderSentAt) {
    await redis.sadd(PENDING_LEADS_SET, cleanEmail);
  }
  return next;
}

/** Marque un lead comme abouti — appelée par auth-signup.js à la création réussie du compte. */
export async function markSignupLeadCompleted(email) {
  const cleanEmail = normalizeEmail(email);
  ...
  const existing = await redis.get(key);
  if (existing) {
    await redis.set(key, { ...existing, completed: true });
  }
  await redis.srem(PENDING_LEADS_SET, cleanEmail);
}

/**
 * Leads à relancer maintenant : ni complétés, ni déjà relancés, et dont la
 * première apparition remonte à au moins `minAgeMs` (le temps de laisser
 * une vraie chance de finir tout seul) mais pas plus de `maxAgeMs` (pas la
 * peine de relancer quelqu'un qui a arrêté il y a des semaines — voir le
 * cron dans pages/api/cron/abandoned-signup-emails.js).
 */
export async function getPendingSignupLeadsForReminder(minAgeMs, maxAgeMs) {
  const redis = getRedis();
  const emails = await redis.smembers(PENDING_LEADS_SET);
  if (!emails.length) return [];
  const records = await Promise.all(emails.map((email) => redis.get(`signup-lead:${email}`)));
  const now = Date.now();
  return records.filter((lead) => {
    if (!lead || lead.completed || lead.reminderSentAt) return false;
    const age = now - lead.firstSeenAt;
    return age >= minAgeMs && age <= maxAgeMs;
  });
}

/** Marque le rappel comme envoyé — évite qu'il reparte une 2e fois. */
export async function markSignupLeadReminderSent(email) {
  const cleanEmail = normalizeEmail(email);
  if (!cleanEmail) return;
  const redis = getRedis();
  const key = `signup-lead:${cleanEmail}`;
  const existing = await redis.get(key);
  if (existing) {
    await redis.set(key, { ...existing, reminderSentAt: Date.now() });
  }
  await redis.srem(PENDING_LEADS_SET, cleanEmail);
}

// ---------------------------------------------------------------------
// Notifications & automatisations (onglet "Notifications" > section
// "Automatisations") : demande d'avis Google déclenchée automatiquement
// après un passage, et relance d'un client devenu inactif. Tout est
// réglable par le commerçant, avec des valeurs par défaut pour que ça
// marche sans réglage — voir lib/notifications.js pour la logique de
// déclenchement, pages/api/notification-settings.js pour l'API, et le
// README pour la configuration de QStash (l'envoi programmé à l'heure
// exacte) et de la relance cron quotidienne pour les clients inactifs.
// ---------------------------------------------------------------------

const DEFAULT_NOTIFICATION_SETTINGS = {
  // Demande d'avis Google automatique.
  reviewRequestEnabled: true,
  // Nombre de passages (création de la carte comptée comme le 1er) après
  // lequel la demande part — 2 par défaut : "1h après le 2e scan, pas celui
  // qui crée la carte" (donc le tout premier retour du client).
  reviewRequestTriggerVisit: 2,
  reviewRequestDelayHours: 1,
  reviewRequestTitle: "Vous avez aimé votre expérience ? ⭐",
  reviewRequestBody: "Donnez votre avis Google et dites-nous ce que vous en avez pensé.",
  reviewRequestButtonLabel: "Laisser un avis Google",
  // Ne redemande pas un avis à un client régulier à chaque passage : il
  // faut attendre ce délai depuis la dernière demande, ET ne jamais
  // dépasser reviewRequestMaxAsks demandes au total pour un même client.
  reviewRequestCooldownDays: 90,
  reviewRequestMaxAsks: 2,
  // Relance "client inactif".
  winbackEnabled: true,
  winbackInactivityDays: 21,
  winbackCooldownDays: 60,
  winbackTitle: "On ne vous a pas vu récemment ! 👋",
  winbackBody: "Revenez vite, votre carte de fidélité vous attend toujours.",
};
...
export async function getNotificationSettings(merchantId) {
  const redis = getRedis();
  const stored = await redis.get(`notif-settings:${merchantId}`);
  return { ...DEFAULT_NOTIFICATION_SETTINGS, ...(stored || {}) };
}

export async function saveNotificationSettings(merchantId, data) {
  const redis = getRedis();
  const current = await getNotificationSettings(merchantId);
  const d = data || {};
  const next = {
    reviewRequestEnabled: d.reviewRequestEnabled !== undefined ? !!d.reviewRequestEnabled : current.reviewRequestEnabled,
    reviewRequestTriggerVisit:
      d.reviewRequestTriggerVisit !== undefined
        ? clampInt(d.reviewRequestTriggerVisit, { min: 1, max: 50, fallback: current.reviewRequestTriggerVisit })
        : current.reviewRequestTriggerVisit,
    reviewRequestDelayHours:
      d.reviewRequestDelayHours !== undefined
        ? clampInt(d.reviewRequestDelayHours, { min: 0, max: 168, fallback: current.reviewRequestDelayHours })
        : current.reviewRequestDelayHours,
    reviewRequestTitle:
      d.reviewRequestTitle !== undefined ? (d.reviewRequestTitle || "").trim().slice(0, 60) || current.reviewRequestTitle : current.reviewRequestTitle,
    reviewRequestBody:
      d.reviewRequestBody !== undefined ? (d.reviewRequestBody || "").trim().slice(0, 300) || current.reviewRequestBody : current.reviewRequestBody,
    reviewRequestButtonLabel:
      d.reviewRequestButtonLabel !== undefined
        ? (d.reviewRequestButtonLabel || "").trim().slice(0, 40) || current.reviewRequestButtonLabel
        : current.reviewRequestButtonLabel,
    reviewRequestCooldownDays:
      d.reviewRequestCooldownDays !== undefined
        ? clampInt(d.reviewRequestCooldownDays, { min: 1, max: 365, fallback: current.reviewRequestCooldownDays })
        : current.reviewRequestCooldownDays,
    reviewRequestMaxAsks:
      d.reviewRequestMaxAsks !== undefined
        ? clampInt(d.reviewRequestMaxAsks, { min: 1, max: 20, fallback: current.reviewRequestMaxAsks })
        : current.reviewRequestMaxAsks,
    winbackEnabled: d.winbackEnabled !== undefined ? !!d.winbackEnabled : current.winbackEnabled,
    winbackInactivityDays:
      d.winbackInactivityDays !== undefined
        ? clampInt(d.winbackInactivityDays, { min: 1, max: 365, fallback: current.winbackInactivityDays })
        : current.winbackInactivityDays,
    winbackCooldownDays:
      d.winbackCooldownDays !== undefined
        ? clampInt(d.winbackCooldownDays, { min: 1, max: 365, fallback: current.winbackCooldownDays })
        : current.winbackCooldownDays,
    winbackTitle: d.winbackTitle !== undefined ? (d.winbackTitle || "").trim().slice(0, 60) || current.winbackTitle : current.winbackTitle,
    winbackBody: d.winbackBody !== undefined ? (d.winbackBody || "").trim().slice(0, 300) || current.winbackBody : current.winbackBody,
  };
  await redis.set(`notif-settings:${merchantId}`, next);
  return next;
}

/**
 * Marque une demande d'avis comme "programmée" tout de suite après l'avoir
 * envoyée à QStash (voir lib/notifications.js) — avant même qu'elle ne
 * parte réellement, pour ne pas en programmer une deuxième si le client
 * rescanne sa carte pendant le délai d'attente (ex : deux passages la même
 * heure). `delayHours` sert à calculer jusqu'à quand ce verrou est valable.
 */
export async function markReviewRequestPending(merchantId, objectId, delayHours) {
  const redis = getRedis();
  const record = await getOwnedClientRecord(redis, merchantId, objectId);
  if (!record) return null;
  record.reviewRequestPendingUntil = Date.now() + Math.max(0, delayHours) * 3600 * 1000 + 30 * 60 * 1000; // +30 min de marge
  await redis.set(`client:${objectId}`, record);
  return record;
}

/** Appelé quand la demande d'avis part réellement (voir pages/api/notify/review-request.js). */
export async function markReviewRequestSent(merchantId, objectId) {
  const redis = getRedis();
  const record = await getOwnedClientRecord(redis, merchantId, objectId);
  if (!record) return null;
  record.reviewRequestCount = (record.reviewRequestCount || 0) + 1;
  record.reviewRequestSentAt = Date.now();
  record.reviewRequestPendingUntil = null;
  await redis.set(`client:${objectId}`, record);
  return record;
}

/** Annule le verrou "demande programmée" sans compter d'envoi (avis déjà laissé, fonctionnalité désactivée entre-temps, etc. — voir pages/api/notify/review-request.js). */
export async function clearReviewRequestPending(merchantId, objectId) {
  const redis = getRedis();
  const record = await getOwnedClientRecord(redis, merchantId, objectId);
  if (!record) return null;
  record.reviewRequestPendingUntil = null;
...
/**
 * Enregistre le clic sur le lien "Laisser un avis" envoyé par email (voir
 * pages/api/review-redirect.js) — utilise getClientByObjectId (pas besoin
 * du merchantId, pas encore connu à ce stade de l'URL de suivi).
 */
export async function markReviewClicked(objectId) {
  const redis = getRedis();
  const record = await redis.get(`client:${objectId}`);
  if (!record) return null;
  record.reviewClickedAt = Date.now();
  await redis.set(`client:${objectId}`, record);
  return record;
}

/** Appelé par le cron quotidien de relance "client inactif" (voir pages/api/cron/winback-notifications.js). */
export async function markWinbackSent(merchantId, objectId) {
  const redis = getRedis();
  const record = await getOwnedClientRecord(redis, merchantId, objectId);
  if (!record) return null;
  record.winbackSentAt = Date.now();
  record.winbackCount = (record.winbackCount || 0) + 1;
  await redis.set(`client:${objectId}`, record);
  return record;
}
