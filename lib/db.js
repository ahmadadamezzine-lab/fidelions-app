...
/**
 * Génère et stocke un code de vérification à 4 chiffres (changement de mot
 * de passe) — valable 10 minutes, avec un compteur de tentatives pour
 * limiter le bruteforce (voir verifyAndConsumePasswordResetCode). Le code
 * est renvoyé à l'appelant (qui l'envoie par email) mais n'est JAMAIS
 * renvoyé dans une réponse API.
 */
export async function createPasswordResetCode(merchantId) {
  const redis = getRedis();
...
/**
 * Vérifie le code saisi par le commerçant et le consomme (usage unique) en
 * cas de succès. Au-delà de 5 tentatives, le code est invalidé d'office —
 * il faut recommencer depuis le début (mot de passe actuel → nouveau code).
 */
export async function verifyAndConsumePasswordResetCode(merchantId, submittedCode) {
  const redis = getRedis();
  const key = `pwreset:${merchantId}`;
  ...
  if (String(submittedCode || "").trim() === stored.code) {
    await redis.del(key);
    return true;
  }
  await redis.set(key, { ...stored, attempts: (stored.attempts || 0) + 1 }, { ex: 600 });
  return false;
}
...
export async function updateLoyaltySettings(merchantId, { tiers }) {
  const redis = getRedis();
